  ...
  return (
    <div className="px-4 py-3 space-y-1.5">
      <div className="flex items-center gap-2">
        ...
        {(t.status === "uploading" || t.status === "queued") && <UploadIcon className="h-4 w-4 shrink-0 text-blue-500" />}
        ...
        <span className={cn("text-xs shrink-0",
          ...
          (t.status === "uploading" || t.status === "queued") && "text-muted-foreground",
        )}>
          {t.status === "complete"  && "Done"}
          {t.status === "error"     && "Failed"}
          {t.status === "queued"    && "Queued"}
          {t.status === "uploading" && `${formatBytes(t.bytesDone)} / ${formatBytes(t.totalBytes)}`}
        </span>
      </div>
      {t.status === "uploading" && (
        <>
